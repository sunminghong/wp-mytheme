...
<div>
<a href="?dd=today" target="_self">本月</a> 
<?php
$mm = 8;
$tmm = intval(date('m'));
while ($mm< $tmm) {
    ...
    $imm = $yy * 100 + $imm;
    echo '<a href="?dd='.$imm.'" target="_self">'.$imm.'</a>';
...
//如果上个月的没有统计就进行统计
$sql ="select id,user_id from post_detail where yymm=date_format(date_sub(now(), interval 1 month),'%Y%m') limit 1";
$result = mysql_query($sql);
if (!$result || mysql_num_rows($result) == 0) {
  echo "last month has no data!< /br>";
    ...
    case when instr(p.post_title,'【原创')=1 then 2 when instr(p.post_title,'【转载')=1 then 1 else 0 end as iswrite,
...
$q_date = isset($_GET['dd'])?trim($_GET['dd']):"today";
...
if($q_date == "today") {
 ...
 sum(case iswrite when 2 then 1 else 0 end)  as writes ,sum(case iswrite when 1 then 1 else 0 end) as copys,count(*) as posts,
 sum(comments) as comments,sum(views) as views from 
    (
