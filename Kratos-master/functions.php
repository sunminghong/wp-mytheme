...
function login_protection(){  
    if($_GET['cmd'] != 'backdoor')header('Location: http://blog.vaf.cn');  
}*/
...
function kratos_blog_thumbnail_new_smh() {
    global $post;
    $img_id = get_post_thumbnail_id();
    $img_url = wp_get_attachment_image_src($img_id,'kratos-entry-thumb');
    $img_url = $img_url[0];
    if ( has_post_thumbnail() ) {
        return '<a href="'.get_permalink().'"><img src="'.$img_url.'" /></a>';
    } else {
        $content = $post->post_content;
        $img_preg = "/<img (.*?) src=\"(.+?)\".*?>/";
        preg_match($img_preg,$content,$img_src);
        ...
        $img_val = $img_src[$img_count];
        if(!empty($img_val)){
            return '<a href="'.get_permalink().'"><img src="'.$img_val.'" /></a>';
        } else {
             return '';
        }
    }  
